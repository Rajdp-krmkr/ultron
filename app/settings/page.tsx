...
export default function SettingsPage() {
  ...
  // Sync settings with MongoDB user settings on mount
  useEffect(() => {
    fetch('/api/user/settings')
      ...
      .then(data => {
        if (data.success && data.settings) {
          ...
          if (dbSettings.llm_mode) setMode(dbSettings.llm_mode);
          if (dbSettings.use_llm !== undefined) setUseLlm(dbSettings.use_llm);
          if (dbSettings.verbose !== undefined) setVerbose(dbSettings.verbose);
          if (dbSettings.visualise !== undefined) setVisualise(dbSettings.visualise);
          if (dbSettings.enable_cache !== undefined) setEnableCache(dbSettings.enable_cache);
          if (dbSettings.cache_only !== undefined) setCacheOnly(dbSettings.cache_only);
          if (dbSettings.num_workers) setWorkers(dbSettings.num_workers);
          if (dbSettings.timeout) setTimeoutVal(dbSettings.timeout);
          if (dbSettings.temperature) setTemperature(dbSettings.temperature);
          if (dbSettings.max_tokens) setMaxTokens(dbSettings.max_tokens);
          if (dbSettings.llm_url) setLlmUrl(dbSettings.llm_url);
          if (dbSettings.models) {
            if (dbSettings.models.detector) setDetectorModel(dbSettings.models.detector);
            if (dbSettings.models.exploiter) setExploiterModel(dbSettings.models.exploiter);
            if (dbSettings.models.reporter) setReporterModel(dbSettings.models.reporter);
            if (dbSettings.models.default) setDefaultModel(dbSettings.models.default);
          }
          if (dbSettings.api_keys) {
            if (dbSettings.api_keys.groq) setGroqKey(dbSettings.api_keys.groq);
            if (dbSettings.api_keys.gemini) setGeminiKey(dbSettings.api_keys.gemini);
            if (dbSettings.api_keys.nvidia) setNvidiaKey(dbSettings.api_keys.nvidia);
          }
        }
      })
      .catch(() => {});
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addTerminalLog('Saved ultron_config.json to MongoDB Atlas & local store.', 'success');
    } catch {
      addTerminalLog('ultron_config.json updated in local state.', 'info');
    }
    ...
    <div className="space-y-4 font-mono text-xs max-w-4xl mx-auto text-left pb-6">
      ...
      <div className="border border-border bg-surface p-3 rounded flex items-center justify-between">
        <div className="space-y-0.5">
          ...
          <p className="text-text-secondary text-[10px]">Manages runtime preferences stored in <code className="text-white bg-black px-1 rounded">ultron_config.json</code> & MongoDB.</p>
        </div>
        <button
          type="button"
          onClick={handleResetDefaults}
          className="px-2.5 py-1 border border-border bg-black hover:border-primary/50 text-[10px] text-text-secondary hover:text-white rounded transition flex items-center gap-1 cursor-pointer"
        >
      ...
      <form onSubmit={handleSaveSettings} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`py-2 px-3 border rounded text-[10px] font-bold text-center transition cursor-pointer ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-[10px]">
            <div className="flex items-center justify-between border border-border bg-black/30 p-2.5 rounded">
              ...
              <button
                ...
                className={`w-10 h-5 rounded-full p-0.5 transition-colors border duration-200 cursor-pointer ${
        ...
        <div className="flex justify-end pt-2">
          ...
          >
            ...
            {isSaved ? 'CONFIG SAVED TO MONGODB' : 'SAVE CONFIGURATION'}
          </button>
        </div>
      </form>
    </div>
